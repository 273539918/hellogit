...
然后，Linux内核就会把这个数据帧封装进一个UDP包里发出去。不过，不要忘了，一个flannel.1设备只知道另一端的flannel.1设备的MAC地址，却不知道对应的宿主机地址是什么。在这种场景下，flannel.1设备实际上要扮演一个“网桥”的角色，在二层网络进行UDP包的转发。而在Linux内核里面，“网桥”设备进行转发的一句，来自一个叫做FDB的转发数据库。不难想到，这个flannel.1"网桥"对应的FDB信息，也是flanneld进程负责维护的。它的内容可以通过如下命令查看：
